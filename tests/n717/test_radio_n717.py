...
def test_set_lte_radio_mode_n717(gsm_service):
    ...
    """
    logging.info("Starting test_set_lte_radio_mode_n717")
    try:
        expected_messages = ["LTE", "EDGE", "GPRS"]
...
def test_set_gsm_radio_mode_n717(gsm_service):
    ...
    """
    logging.info("Starting test_set_gsm_radio_mode_n717")
    try:
...
def test_set_pref_bands_to_2g_n717(gsm_service):
    logging.info("Testing setting preferred bands to GSM (2G)...")
    ...
    assert read_value == f"pref_bands={gsm_pref_bands}", f"Expected pref_bands={gsm_pref_bands}, but got {read_value}"

    logging.info(f"Preferred bands correctly set to {read_value}")
    logging.info("Test for setting preferred bands to GSM (2G) completed successfully.")


def test_tcp_port_change_n717(gsm_service):
    logging.info("Testing changing TCP port value...")
    tcp_port_value = '1234'

    gsm_service.login_admin()
    logging.info(f"Changing TCP port to {tcp_port_value}")
    ...
    gsm_service.login_admin()
    expected_message = f"lct_tcp_port={tcp_port_value}"
